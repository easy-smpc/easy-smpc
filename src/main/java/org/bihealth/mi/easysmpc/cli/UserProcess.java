        ...
        for (int index = 0; index < getModel().getNumParticipants(); index++) {
            ...
            if (index != getModel().getOwnId()) {
                ...
                try {
                    ...
                    future = getModel().getBus(getModel().getConnectionIMAPSettings().getCheckInterval(), false).send(new org.bihealth.mi.easybus.Message(Message.serializeMessage(getModel().getUnsentMessageFor(index))),
