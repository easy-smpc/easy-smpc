...
public class BusEasyBackend extends Bus {
    ...
    protected void receive() throws BusException, InterruptedException {
        ...
        for (String scope : getScopesForParticipant(self)) {
            ...
            if(exception != null) {
                LOGGER.error("Unable to get messages for " + self.getIdentifier());
                continue;
            }
    ...
    private void send(Participant receiver, Scope scope, Object message) throws BusException {
        ...
        // Send message
        try {
            new HTTPRequest(server, String.format(PATH_SEND_MESSAGE_PATTERN, scope.getName(), receiver.getIdentifier()), HTTPRequestType.POST, getToken(), body).execute();
        } catch (HTTPException e) {
            
            // Existing initial messages error
            if(e.getStatusCode() == 418) {
                LOGGER.error(String.format("Tried to add an illegal second initial message for scope %s and receiver %s!", scope.getName(), receiver.getIdentifier()));
                throw new IllegalStateException(String.format("Tried to add an illegal second initial message for scope %s and receiver %s!", scope.getName(), receiver.getIdentifier()));
            }
